                ...
                <div class="flex flex-wrap items-end gap-4">
                        ...
                        <select name="sport_id" id="sport_id"
                            ...
                            @foreach ($sports as $sport)
                                <option value="{{ $sport->Sport_ID }}"
                                    {{ $sportId == $sport->Sport_ID ? 'selected' : '' }}>
                        ...
                        <select name="camp_id" id="camp_id"
                            ...
                            @foreach ($camps as $camp)
                                <option value="{{ $camp->Camp_ID }}" {{ $campId == $camp->Camp_ID ? 'selected' : '' }}>
                                    {{ $camp->Camp_Name }} @if ($camp->sport && $camp->sport->Sport_Name)
                                        ({{ $camp->sport->Sport_Name }})
                                    @endif
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex-1 min-w-48">
                        ...
                        <select name="payment_status" id="payment_status"
                            ...
                            <option value="not_paid" {{ $paymentStatus == 'not_paid' ? 'selected' : '' }}>Not Fully
                                Paid</option>
                            <option value="partial" {{ $paymentStatus == 'partial' ? 'selected' : '' }}>Partially Paid
                            </option>
                    ...
                    <div class="flex gap-3" style="margin-top: 20px;">
                        <button type="submit" class="filter-btn"
                            style="background-color: #2563eb; color: white; padding: 6px 16px; border: none; border-radius: 6px; font-weight: 500; cursor: pointer; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); white-space: nowrap; height: 38px; display: flex; align-items: center; justify-content: center;">
                            Filter Orders
                        </button>

                        @if ($sportId || $campId || $paymentStatus)
                            <a href="{{ route('admin.finances') }}" class="clear-btn"
                                style="background-color: #6b7280; color: white; padding: 6px 16px; border-radius: 6px; font-weight: 500; text-decoration: none; display: flex; align-items: center; justify-content: center; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); white-space: nowrap; height: 38px;">
                                Clear Filters
                            </a>
                        @endif
                    </div>
            ...
            <div class="px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                ...
                @if ($orders->count() > 0)
                    <form method="POST" action="{{ route('admin.finances.export') }}" style="display: inline;">
                        ...
                        <button type="submit" class="export-btn text-sm"
                            style="background-color: #2563eb; color: white; padding: 6px 12px; border: none; border-radius: 6px; font-weight: 500; cursor: pointer; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); transition: all 0.2s ease-in-out;">
                            Export to Excel
                        </button>
                    </form>
                @endif
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($orders as $order)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900">
                                        @if ($order->parent)
                                            {{ $order->parent->Parent_FirstName }}
                                            {{ $order->parent->Parent_LastName }}
                                        @else
                                            <span class="text-gray-500">N/A</span>
                                        @endif
                                    </td>
